...
class FireflyFile extends PolymerElement {
  ...
  static get template() {
    return html`
      ...
      <paper-fab icon="aspen:upload" on-click="__onFileIconClick"></paper-fab
      ><label>[[label]]</label>
      <firebase-auth app-name="[[appName]]"></firebase-auth>
      <input class="file-upload-input" type="file" on-change="__fileSelected" />
  ...
  /**
   * handles the click event on upload icon
   */
  __onFileIconClick() {
   ...
   * @param {Object} e the file select event
   */
  __fileSelected(e) {
    e.preventDefault();
    e.stopPropagation();
